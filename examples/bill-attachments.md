...
实际部署可采用 1 MiB 分块，给 Base64 编码和请求封装留出余量，避免超过 Web 服务器请求体限制。
最后一块可能触发较慢的远端文件存储。附件传输默认等待 120 秒，可通过
`KINGDEE_ATTACHMENT_TIMEOUT` 设置有限正数秒数；连接超时仍为 30 秒。超时后仍须先核查附件记录，
不能因为延长了等待时间就自动重传。
...
- 同一单据允许多个同名附件：测试账套中，不传 `file_id` 再次上传同名文件会创建独立附件，
  原附件内容保持不变。不能以文件名作为唯一标识或把同名上传当作覆盖；应保存 FileId 和附件内码。
  恢复未知结果时，先按单据核查记录，再下载候选附件比对大小与内容哈希；如果仍有多个候选，
  不能仅凭文件名认定已恢复，也不要自动再次上传。
...
2026-08-31 已在独立测试账套验证采购订单草稿的 TXT、小 PDF 和中文文件名大 PDF 表头附件。
大 PDF 共 6,604,459 字节，以 1 MiB 分成 7 块上传，服务端同样返回 7 个下载分块；字节内容和
SHA-256 一致，中文、空格、全角括号及中文别名完整保留。最后一块存储耗时约 76 秒。
单据仍为创建状态。分录附件目前仍以 HTTP mock 测试验证，发布前需补充真实账套联调。

同日补充边界验证：

| 场景 | 验证方式与结果 |
|------|----------------|
| 未登录下载 | 无 Cookie 请求测试附件，服务端返回会话丢失，未返回文件内容 |
| 角色权限限制 | 尚未实测；需要另行配置受限账号。权限不足响应的保留和不重试行为已有 mock 测试 |
| 错误参数 | 真实接口拒绝不存在的 FileId、超出文件末尾的下载位置；非法 Base64、负游标等由本地模型校验测试覆盖 |
| 同名附件 | 同一单据下生成两个不同 FileId，分别下载后确认新旧内容均正确 |
| 响应丢失与恢复 | 真实上传完成后在客户端故意丢弃响应，注入 ReadTimeout；工具返回结果未知。随后仅查询和下载，找到唯一附件并验证 SHA-256，整个测试只发出一次上传请求 |

响应丢失测试是客户端故障注入，并不代表复现了真实网络中断。上述验证结束后测试草稿共有
5 个附件，仍未提交或审核；没有修改账号权限，也没有删除测试附件。
